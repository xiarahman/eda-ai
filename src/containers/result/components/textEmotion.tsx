...
const EmotionCard = () => {
  ...
  const availableEmotions = result.top_three_emotions.map(emotion => emotion.emotion);
  ...
  return (
    <Card style={{ backgroundColor: "#f5f5f5", width:'600px', display: 'flex', flexDirection:'column', }}>
     <Text type={"p"} className="emotions-card">Emotions</Text>
      <Flex vertical align="center" >
       
        <ProgressBar
          // Set the value of the progress bar based on the selected emotion
          value={Math.floor(result.top_three_emotions.find(
            (emotion) => emotion.emotion.toLowerCase() === selectedEmotion?.toLowerCase()
          )?.percentage ?? 0)}
